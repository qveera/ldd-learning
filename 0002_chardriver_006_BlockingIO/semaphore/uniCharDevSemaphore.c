...
#define DEV_NAME "uniCharDevSemaphore" //This will be shown in cat /proc/devices
...
ssize_t uni_read(struct file *file, char __user *buf, size_t lbuf, loff_t *ppos) {
	...
	if (down_interruptible (&uni_sem)) {
		printk (KERN_INFO "%s[%d]: process %i woken up by a signal\n",__func__,__LINE__,current->pid);
		return -ERESTARTSYS;
	}
...
static int __init uni_char_dev_init(void) {
	...
	//Init the semaphore with state locked
