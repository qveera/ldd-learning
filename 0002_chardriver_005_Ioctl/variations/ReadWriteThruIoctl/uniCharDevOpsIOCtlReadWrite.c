...
#define DEV_NAME "uniCharDevOpsIOCtlReadWrite" //This will be shown in cat /proc/devices
...
long uni_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
	...
	switch (cmd) {
		...
		case UNI_GET_SIZE:
			...
			break;
		case UNI_WRITE_BUF:
			copy_from_user(&bi, (struct buf_info *)arg, sizeof(struct buf_info));
			uni_write(file, bi.buf, bi.size, &file->f_pos);
			break;
		case UNI_READ_BUF:
			copy_from_user(&bi, (struct buf_info *)arg, sizeof(struct buf_info));
			uni_read(file, bi.buf, bi.size, &file->f_pos);
			break;
	}
	return SUCCESS;
}
